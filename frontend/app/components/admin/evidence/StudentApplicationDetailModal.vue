<template>
  <UModal
    :open="open"
    title="Duyệt Hồ Sơ Minh Chứng"
    description="Xem nội dung chi tiết các tiêu chí và đánh giá minh chứng của sinh viên."
    ...
    <template #body>
      ...
      >
        ...
        <p class="mt-3 text-sm text-[#64748B] font-medium">
          Đang tải hồ sơ minh chứng sinh viên...
        </p>
      </div>
            ...
            <div class="flex flex-wrap items-center gap-x-6 gap-y-2">
              <div>
                ...
                <p class="mt-0.5 font-bold text-sm text-[#1E293B]">
                  {{ application?.studentName || 'Chưa cập nhật' }}
                </p>
              </div>
              ...
              <div>
                ...
                <p class="mt-0.5 text-xs text-[#334155] font-medium">
...
import type { StudentApplication, StandardResultDTO, CriteriaResultDTO } from '~/types/admin'
...
const selectedCriteria = ref<CriteriaResultDTO | null>(null)
const selectedStandard = ref<StandardResultDTO | null>(null)
...
function handleSelectCriteria(data: { criteria: CriteriaResultDTO, standard: StandardResultDTO }) {
...
// Forward action emits to parent page
function handleApprove(data: { criteria: CriteriaResultDTO, comment: string }) {
  emit('approve', data)
}

function handleReject(data: { criteria: CriteriaResultDTO, reason: string }) {
  emit('reject', data)
}

function handleSaveComment(data: { criteria: CriteriaResultDTO, comment: string }) {
  emit('save-comment', data)
}
...
const stats = computed(() => {
  ...
  const processCriteria = (c: CriteriaResultDTO) => {
